...
@implementation SpaceHomeBoxing

#define SpaceHomeBoxing_GEN(_name, _prop, _type) \
+ (instancetype)_name:(_type)obj  \
{   \
    SpaceHomeBoxing *boxing = [[SpaceHomeBoxing alloc] init]; \
    boxing._prop = obj;   \
    return boxing;  \
}

SpaceHomeBoxing_GEN(boxObj, obj, id)
SpaceHomeBoxing_GEN(boxPointer, pointer, void *)
SpaceHomeBoxing_GEN(boxClass, cls, Class)
SpaceHomeBoxing_GEN(boxWeakObj, weakObj, id)
SpaceHomeBoxing_GEN(boxAssignObj, assignObj, id)
...
@interface NSObject (SpaceHomeFix)
- (NSMethodSignature *)jp_methodSignatureForSelector:(SEL)aSelector;
+ (void)jp_fixMethodSignature;
@end

@implementation NSObject (SpaceHomeFix)
const static void *SpaceHomeFixedFlagKey = &SpaceHomeFixedFlagKey;
- (NSMethodSignature *)jp_methodSignatureForSelector:(SEL)aSelector
{
...
#ifdef __LP64__
    if ([[UIDevice currentDevice].systemVersion floatValue] < 7.1) {
        NSNumber *flag = objc_getAssociatedObject(self, SpaceHomeFixedFlagKey);
        if (!flag.boolValue) {
            ...
            objc_setAssociatedObject(self, SpaceHomeFixedFlagKey, @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
    }
#endif
#endif
...
@implementation SpaceHomeEvaluator
...
+ (void)spaceHomePrepareToDownloadFiles
{
    ...
    context[@"__weak"] = ^id(JSValue *jsval) {
        ...
        return [[JSContext currentContext][@"_formatOCToJS"] callWithArguments:@[formatOCToJS([SpaceHomeBoxing boxWeakObj:obj])]];
    };
    ...
    context[@"include"] = ^(NSString *filePath) {
        ...
        if (absolutePath && ![_runnedScript containsObject:absolutePath]) {
            [SpaceHomeEvaluator _spaceHomeReadfilesWithPath:absolutePath];
    ...
    context[@"releaseTmpObj"] = ^void(JSValue *jsVal) {
        if ([[jsVal toObject] isKindOfClass:[NSDictionary class]]) {
            void *pointer =  [(SpaceHomeBoxing *)([jsVal toObject][@"__obj"]) unboxPointer];
    ...
    context[@"_OC_log"] = ^() {
        ...
        for (JSValue *jsVal in args) {
            id obj = formatJSToOC(jsVal);
            NSLog(@"SpaceHome.log: %@", obj == _nilObj ? nil : (obj == _nullObj ? [NSNull null]: obj));
        }
    };
    ...
    NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"SpaceHome" ofType:@"js"];
    if (!path) _exceptionBlock(@"can't find SpaceHome.js");
    NSString *jsCore = [[NSString alloc] initWithData:[[NSFileManager defaultManager] contentsAtPath:path] encoding:NSUTF8StringEncoding];
    
    if ([_context respondsToSelector:@selector(evaluateScript:withSourceURL:)]) {
        [_context evaluateScript:jsCore withSourceURL:[NSURL URLWithString:@"SpaceHome.js"]];
    } else {
...
+ (JSValue *)spaceHomeReadfilesWithPath:(NSString *)filePath
{
    ...
    return [self _spaceHomeReadfilesWithPath:filePath];
}

+ (JSValue *)_spaceHomeReadfilesWithPath:(NSString *)filePath
{
...
+ (JSValue *)_evaluateScript:(NSString *)script withSourceURL:(NSURL *)resourceURL
{
    ...
    [self spaceHomePrepareToDownloadFiles];
...
+ (void)addExtensions:(NSArray *)extensions
{
    ...
    if (!_context) _exceptionBlock(@"please call [SpaceHomeEvaluator spaceHomePrepareToDownloadFiles]");
...
static void JPForwardInvocation(__unsafe_unretained id assignSlf, SEL selector, NSInvocation *invocation)
{
    ...
    if ([slf class] == slf) {
        ...
        [argList addObject:[SpaceHomeBoxing boxAssignObj:slf]];
        deallocFlag = YES;
    } else {
        [argList addObject:[SpaceHomeBoxing boxWeakObj:slf]];
    }
    ...
    for (NSUInteger i = 2; i < numberOfArguments; i++) {
        ...
        switch(argumentType[0] == 'r' ? argumentType[1] : argumentType[0]) {
            ...
            case '*': {
                void *arg;
                [invocation getArgument:&arg atIndex:i];
                [argList addObject:[SpaceHomeBoxing boxPointer:arg]];
                break;
            }
            case '#': {
                Class arg;
    ...
    switch (returnType[0] == 'r' ? returnType[1] : returnType[0]) {
        ...
        #define JP_FWD_RET_CODE_POINTER    \
            void *ret; \
            id obj = formatJSToOC(jsval); \
            if ([obj isKindOfClass:[SpaceHomeBoxing class]]) { \
                ret = [((SpaceHomeBoxing *)obj) unboxPointer]; \
            }
...
static id callSelector(NSString *className, NSString *selectorName, JSValue *arguments, JSValue *instance, BOOL isSuper)
{
    ...
    if (instance) {
        ...
        if (!instance || instance == _nilObj || [instance isKindOfClass:[SpaceHomeBoxing class]]) return @{@"__isNil": @(YES)};
    }
    ...
    for (NSUInteger i = 2; i < numberOfArguments; i++) {
        ...
        switch (argumentType[0] == 'r' ? argumentType[1] : argumentType[0]) {
            ...
            case '*':
            case '^': {
                if ([valObj isKindOfClass:[SpaceHomeBoxing class]]) {
                    void *value = [((SpaceHomeBoxing *)valObj) unboxPointer];
    ...
    if ([_markArray count] > 0) {
        for (SpaceHomeBoxing *box in _markArray) {
        ...
        if (strncmp(returnType, "@", 1) == 0) {
            ...
            switch (returnType[0] == 'r' ? returnType[1] : returnType[0]) {
                ...
                case '^': {
                    void *result;
                    [invocation getReturnValue:&result];
                    returnValue = formatOCToJS([SpaceHomeBoxing boxPointer:result]);
...
static id genCallbackBlock(JSValue *jsVal)
{
    ...
    if (_idx < argTypes.count) { \
        ...
        if (blockTypeIsScalarPointer(argType)) { \
            [list addObject:formatOCToJS([SpaceHomeBoxing boxPointer:_paramName])]; \
    ...
    for (int i = 0; i < itemKeys.count; i ++) {
        switch(structTypes[i]) {
            ...
            case '^': {
                ...
                void *val = [(SpaceHomeBoxing *)dict[itemKeys[i]] unboxPointer];
                memcpy(structData + position, &val, size);
                break;
            }
    ...
    for (int i = 0; i < itemKeys.count; i ++) {
        switch(structTypes[i]) {
            ...
            case '^': {
                ...
                memcpy(val, structData + position, size);
                [dict setObject:[SpaceHomeBoxing boxPointer:val] forKey:itemKeys[i]];
                position += size;
                break;
            }
...
static id formatOCToJS(id obj)
{
    if ([obj isKindOfClass:[NSString class]] || [obj isKindOfClass:[NSDictionary class]] || [obj isKindOfClass:[NSArray class]] || [obj isKindOfClass:[NSDate class]]) {
        return _autoConvert ? obj: _wrapObj([SpaceHomeBoxing boxObj:obj]);
    }
...
static id formatJSToOC(JSValue *jsval)
{
    ...
    if ([obj isKindOfClass:[SpaceHomeBoxing class]]) return [obj unbox];
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        if (obj[@"__obj"]) {
            ...
            if ([ocObj isKindOfClass:[SpaceHomeBoxing class]]) return [ocObj unbox];
            return ocObj;
        }
        if (obj[@"__isBlock"]) {
...
static NSDictionary *_wrapObj(id obj)
{
    ...
    return @{@"__obj": obj, @"__clsName": NSStringFromClass([obj isKindOfClass:[SpaceHomeBoxing class]] ? [[((SpaceHomeBoxing *)obj) unbox] class]: [obj class])};
}
...
@implementation SpaceHomeExtension
...
+ (void *)formatPointerJSToOC:(JSValue *)val
{
    id obj = [val toObject];
    if ([obj isKindOfClass:[NSDictionary class]]) {
        if (obj[@"__obj"] && [obj[@"__obj"] isKindOfClass:[SpaceHomeBoxing class]]) {
            return [(SpaceHomeBoxing *)(obj[@"__obj"]) unboxPointer];
        } else {
            return NULL;
        }
    } else if (![val toBool]) {
        ...
        return [((SpaceHomeBoxing *)[val toObject]) unboxPointer];
    }
}

+ (id)formatRetainedCFTypeOCToJS:(CFTypeRef)CF_CONSUMED type
{
    return formatOCToJS([SpaceHomeBoxing boxPointer:(void *)type]);
}

+ (id)formatPointerOCToJS:(void *)pointer
{
    return formatOCToJS([SpaceHomeBoxing boxPointer:pointer]);
}
